            ...
            <div :class="['comment', 'card-body', {'linked-comment': comment.is_linked}]" v-for="comment in comments" :key="comment.id" v-if="!comment.deleted && !(!comment.is_linked && comment.id === {% if request.GET.comment_link %}{{ request.GET.comment_link }}{% else %}none{% endif %})">
                ...
                <div class="d-flex justify-content-between">
                    <small class="text-muted d-flex gap-1 align-items-center">
                        <a class="d-flex align-items-center text-decoration-none" :href="'/user/profile/' + comment.user.slug">
                    ...
                    <div class="dropdown">
                        ...
                        <ul v-else class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            <li class="dropdown-item cursor-pointer" @click="redirectToUser(comment.user.username)">Докладвай</li>
                        </ul>
                    </div>
                </div>
